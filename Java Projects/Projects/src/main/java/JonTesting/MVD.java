...
public class MVD {
    ...
    public static void main(String[] Args) throws IOException {
        ...
        PrintWriter out = new PrintWriter(new FileWriter("C:\\Users\\jonsi\\Desktop\\Mvd.txt"));
        String func;
        Tables.add("Persons");
        Tables.add("Boats");
        Tables.add("Projects");
                ...
                for (int k = 0; k < Keys2.size(); k++)
                {
                    for (int j = 0; j < Keys2.size(); j++)
                    {
                        for (int l = 0; l <Keys2.size(); l++) {
                            ...
                            if (Keys2.get(k) != Keys2.get(j) && Keys2.get(j) != Keys2.get(l) && Keys2.get(l) != Keys2.get(k)) {
                                func = "SELECT " + "'" + Tables.get(i) + ": ";
                                func += Keys2.get(k) + "-->>" + Keys2.get(j) + "; " + Keys2.get(k) + " -->> " +  Keys2.get(l) + "' AS FD, CASE WHEN COUNT(*)=0 THEN 'HOLDS' ELSE 'NOT MVD' END \n";
                                func += "FROM( ";
                                func += "SELECT " + Keys2.get(k) + "\n";
                                func += "FROM " + Tables.get(i) + "\n";
                                func += "GROUP BY " + Keys2.get(k) + "\n";
                                func += "HAVING COUNT(DISTINCT " + Keys2.get(i) + ") * COUNT(DISTINCT " + Keys2.get(j) + ") * COUNT(DISTINCT " + Keys2.get(l) + " )\n";
                                func += "<> COUNT (*)\n";
                                func += ") X;";
                                out.println(func);
                            }
